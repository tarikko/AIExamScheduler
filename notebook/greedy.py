...
# ---------------------------------------------------------------------------
# Test — mirrors the A* test exactly, runs both greedy strategies + A*
# for comparison as required by the project
# ---------------------------------------------------------------------------
...
def test():
    ...
    from_time = "09:00"
    to_time   = "17:00"
    days_str  = ("Mond-01-06-2006, Tues02-06-2006, Wed-03-06-2006, "
                 "Thur-04-06-2006, Sat-06-06-2006, Sun-07-06-2006, Mon-08-06-2006")
...
durations = {
    ...
    first_year_modules = [
        "Linear_algebra", "Statistics", "Introduction_to_linux",
        "oop", "Analyse_1", "English_2", "history_of_algeria",
    ]
    second_year_modules = [
        "introduction_to_AI", "lab_circuit", "analyse_3",
        "computer_architecture", "inferential_statistic",
        "operating_system", "theory_of_computing",
    ]
    third_year_modules = [
        "MAchine_Learning", "Computer_networking_security",
        "Numerical_methods_and_optimizations", "Advanced_databases",
        "Time_Series_Analysis_and_Classification",
    ]
    fourth_year_modules = [
        "Big Data Analytics & Visualization", "Computer Vision",
        "Speech Processing", "Academic Communication and Research",
        "High Performance Computing", "Enterprise Computing",
        "Reinforcement Learning",
    ]
    fifth_year_modules = [
        "Introduction to Business Law", "Introduction to Research Methodology",
        "Advanced Research Topics", "History_of_algeria1",
        "Citizenship", "Market Research",
    ]
    ...
    def run_and_report(label, search_fn, *args):
        """Helper: run one algorithm, print its schedule and stats."""
        print(f"\n{'='*60}")
        print(f"  {label}")
        print('='*60)
        ...
        t0 = time.time()
        result = search_fn(scheduler, *args)
        elapsed = time.time() - t0

        if result == "Success!":
            penalty = scheduler.calculate_g(scheduler.schedule)
            print(f"Result  : {result}")
            print(f"Penalty : {penalty}")
            print(f"Time    : {elapsed:.3f}s")
            print(f"\n{'Exam':<45} {'Day':<18} {'Time':<12} Rooms")
            print('-' * 95)
            for entry in sorted(scheduler.schedule, key=lambda e: (e['day'], e['start'])):
                print(
                    f"{entry['exam']:<45} {entry['day']:<18} "
                    f"{entry['start']}-{entry['end']:<8} {entry['rooms']}"
                )
        else:
            print(f"Result  : {result}")
            print(f"Time    : {elapsed:.3f}s")

return scheduler, elapsed, result

    # ---- run all three algorithms ----
    sched_le,  t_le,  r_le  = run_and_report(
        "Greedy — Largest Enrollment First",
        lambda s: s.greedySearch('largest_enrollment')
    )
    sched_dh,  t_dh,  r_dh  = run_and_report(
        "Greedy — Degree Heuristic",
        lambda s: s.greedySearch('degree_heuristic')
    )
    sched_astar, t_as, r_as = run_and_report(
        "A* Search",
        lambda s: s.AstarSearch()
    )

    # ---- comparative summary (required by the project) ----
    print(f"\n{'='*60}")
    print("  Comparative Summary")
    print('='*60)
    print(f"{'Algorithm':<35} {'Result':<16} {'Penalty':>8} {'Time (s)':>10}")
    print('-' * 72)

    def stats(scheduler, result, elapsed):
        penalty = scheduler.calculate_g(scheduler.schedule) if result == "Success!" else "N/A"
        return result, penalty, f"{elapsed:.3f}"

    for label, s, r, t in [
        ("Greedy – Largest Enrollment", sched_le,    r_le,  t_le),
        ("Greedy – Degree Heuristic",   sched_dh,    r_dh,  t_dh),
        ("A* Search",                   sched_astar, r_as,  t_as),
    ]:
        res, pen, tim = stats(s, r, t)
        print(f"{label:<35} {str(res):<16} {str(pen):>8} {tim:>10}")
