(() => {
  ...
  const timeSlots = [
    { id: 'Mon-09-11', day: 'Monday', label: 'Mon • 09:00 - 11:00', index: 0 },
    { id: 'Mon-11-13', day: 'Monday', label: 'Mon • 11:00 - 13:00', index: 1 },
    { id: 'Mon-13-15', day: 'Monday', label: 'Mon • 13:00 - 15:00', index: 2 },
    { id: 'Tue-09-11', day: 'Tuesday', label: 'Tue • 09:00 - 11:00', index: 3 },
    { id: 'Tue-11-13', day: 'Tuesday', label: 'Tue • 11:00 - 13:00', index: 4 },
    { id: 'Tue-13-15', day: 'Tuesday', label: 'Tue • 13:00 - 15:00', index: 5 },
    { id: 'Wed-09-11', day: 'Wednesday', label: 'Wed • 09:00 - 11:00', index: 6 },
    { id: 'Wed-11-13', day: 'Wednesday', label: 'Wed • 11:00 - 13:00', index: 7 }
  ];
  ...
  function renderDataOverview(data) {
    ...
    const enrollTotal = data.courses.reduce((sum, course) => sum + course.enrollment, 0);
  ...
  function gaSchedule(data) {
    ...
    const combos = timeSlots.flatMap(slot => rooms.map(room => ({ slot, room })));

    function randomIndividual() {
      return data.courses.map(course => {
        const domain = combos.filter(combo => combo.room.capacity >= course.enrollment);
      ...
      for (let i = 0; i < individual.length; i++) {
        for (let j = i + 1; j < individual.length; j++) {
          ...
          if (courseA.slot.id === courseB.slot.id) {
            ...
            const studentsConflict = data.students.some(student =>
              student.courses.includes(data.courses[i].code) && student.courses.includes(data.courses[j].code)
            );
    ...
    const best = population.sort((a, b) => computeFitness(b) - computeFitness(a))[0];
    return best.map((assignment, index) => ({ course: data.courses[index], slot: assignment.slot, room: assignment.room }));
  }

  function aStarSchedule(data) {
    const conflicts = buildConflicts(data);
    const rooms = data.rooms;
    const domain = timeSlots.flatMap(slot => rooms.map(room => ({ slot, room })));
    ...
    while (frontier.length && steps < maxSteps) {
      ...
      const filtered = domain.filter(({ slot, room }) => room.capacity >= nextCourse.enrollment);

      filtered.forEach(choice => {
        const valid = node.assignments.every(assigned => {
          if (assigned.slot.id === choice.slot.id && assigned.room.name === choice.room.name) {
            return false;
          }
          if (assigned.slot.id === choice.slot.id && conflicts[nextCourse.code].has(assigned.course.code)) {
            return false;
          }
          return true;
        });
        if (!valid) return;
        const child = {
          assignments: [...node.assignments, { course: nextCourse, slot: choice.slot, room: choice.room }]
        };
        child.f = child.assignments.length + heuristic(child.assignments.length);
        frontier.push(child);
      });
    }
  ...
  function renderSchedule(assignments, meta) {
    ...
    header.innerHTML += `<div class="grid-cell">${meta.time.toFixed(1)}ms</div>`;
  ...
  function buildMasterGrid(assignments) {
    ...
    timeSlots.forEach(slot => {
      ...
      dataset.rooms.forEach(room => {
        ...
        cell.textContent = match ? `${match.course.code} (${match.course.enrollment})` : '—';
      });
    });
  }
