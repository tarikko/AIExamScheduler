...
export function renderSchedule(assignments, meta) {
	...
	const durationViolations = assignments.filter((a) => a.course.durationMins > a.slot.durationMins);
	if (durationViolations.length > 0) {
		...
		row.innerHTML += `<div class="grid-cell">${durationViolations.slice(0, 2).map(v => `${v.course.code} (${v.course.durationMins}m)` ).join("; ")}</div>`;
		results.appendChild(row);
	}

	// 5. Success Row
	if (roomViolations.length === 0 && studentOverlaps.length === 0 && durationViolations.length === 0) {
		const row = document.createElement("div");
		row.className = "grid-row";
...
export function buildMasterGrid(assignments, rooms, timeSlots) {
	...
	timeSlots.forEach((slot) => {
		...
		rooms.forEach((room) => {
			...
			if (match) {
				const isExceeding = match.course.durationMins > slot.durationMins;
