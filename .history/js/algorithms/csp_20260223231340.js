...
export function cspSchedule(data, onProgress) {
	const slots = data.timeSlots || timeSlots;
	const conflicts = buildConflicts(data);
	const domains = {};
	data.courses.forEach((course) => {
		domains[course.code] = slots.flatMap((slot) =>
			data.rooms
				.filter(
					(room) =>
						room.capacity >= course.enrollment &&
						slot.durationMins >= course.durationMins
				)
				.map((room) => ({ slot, room }))
		);
	});
	...
	function selectCourse(partial) {
		...
		data.courses.forEach((course) => {
			...
			if (!candidate || choices.length < (candidate.choices?.length || 0)) {
	...
	function backtrack(partial) {
		if (onProgress) onProgress((partial.length / data.courses.length) * 100);
		...
		for (const choice of node.choices) {
			const next = backtrack([
				...partial,
				{ course: node.course, slot: choice.slot, room: choice.room },
			]);
			if (next) return next;
		}
		return null;
