...
export function gaSchedule(data, onProgress) {
	...
	function randomIndividual() {
		return data.courses.map((course) => {
			const domain = combos.filter(
				(combo) =>
					combo.room.capacity >= course.enrollment &&
					combo.slot.durationMins >= (course.durationMins || 0)
			);
			// Fallback: if no combo satisfies duration, relax the duration constraint
			const candidates = domain.length > 0 ? domain : combos.filter(
				(combo) => combo.room.capacity >= course.enrollment
			);
			// Last resort: any combo at all
			const pool = candidates.length > 0 ? candidates : combos;
	...
	function computeFitness(individual) {
		...
		for (let i = 0; i < individual.length; i++) {
			...
			if (!assignA || !assignA.slot || !assignA.room) { hard += 20; continue; }
			if ((courseA.durationMins || 0) > assignA.slot.durationMins) hard += 10;

			for (let j = i + 1; j < individual.length; j++) {
				const assignB = individual[j];
				const courseB = data.courses[j];

				if (!assignB || !assignB.slot || !assignB.room) continue;
		...
		const slotIndices = individual
			.filter((a) => a && a.slot)
			.map((assign) => assign.slot.index);
	...
	return best
		.map((assignment, index) => {
			if (!assignment || !assignment.slot || !assignment.room) return null;
			return {
