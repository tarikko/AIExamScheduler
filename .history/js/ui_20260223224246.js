...
export function renderSchedule(assignments, meta) {
	...
	Object.entries(roomConflicts).forEach(([key, codes]) => {
		...
		row.innerHTML = `
			<div class="grid-cell slot-label">[ROOM CONFLICT]</div>
			<div class="grid-cell">${key}</div>
			<div class="grid-cell">${[...new Set(codes)].join(", ")}</div>
		`;
	...
	Object.entries(studentConflicts).forEach(([key, count]) => {
		...
		row.className = "grid-row";
		row.innerHTML = `
			<div class="grid-cell slot-label">[STUDENT CONFLICT]</div>
			<div class="grid-cell">${key}</div>
			<div class="grid-cell">${count} student(s)</div>
		`;
		results.appendChild(row);
	});
	...
	if (meta.softViolations && meta.softViolations > 0) {
		...
		row.className = "grid-row";
		row.innerHTML = `
			<div class="grid-cell slot-label">[STRESS LOAD]</div>
			<div class="grid-cell">Back-to-back exams</div>
			<div class="grid-cell">${meta.softViolations} student(s)</div>
		`;
		results.appendChild(row);
	}
	...
	if (Object.keys(roomConflicts).length === 0 && Object.keys(studentConflicts).length === 0 && (!meta.softViolations || meta.softViolations === 0)) {
		...
		row.className = "grid-row";
		row.innerHTML = `
			<div class="grid-cell slot-label">[OK]</div>
			<div class="grid-cell">No violations</div>
			<div class="grid-cell">Clean schedule</div>
		`;
		results.appendChild(row);
	}

	document.getElementById("score-note").textContent =
		"Total penalty points: " + meta.penalty;
}
